    ...
    return (
        <Card className='h-full max-w-[39rem] p-3 max-lg:w-full lg:w-auto'>
            <CardHeader>
                ...
                <CardDescription className='text-primary-content/90 max-w-[400px]'>{item.description}</CardDescription>
            </CardHeader>
            <CardContent>
                <div className='mb-3 flex h-[5.5rem] items-center'>
                    {item.price !== null && (
                        <>
                            ...
                            <span className='text-[4.0rem] font-bold leading-none md:text-[4.5rem]'>{item.price}</span>
                            {item.hasPriceDesc && <span className='ml-1 text-xl md:text-2xl'>{item.priceDescription}</span>}
                        </>
                    )}
                </div>

                <Link to={item.url}>
                    <Button size={'lg'} className={`mb-7 w-full`}>
                        {item.buttonText}
                    </Button>
                </Link>
                ...
                <ul className='space-y-4'>
                    {item.features.map((feature, index) => (
                        <li key={index} className='border-n-6 flex items-center justify-start border-t pt-4'>
...
export const WeeklyPriceCard: React.FC = () => <PricingCards pricingData={weeklyPricing} />
